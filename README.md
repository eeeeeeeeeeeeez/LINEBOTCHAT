...
## 支援的訊息類型
- **文字**：直接丟給 Gemini 回答，遇到即時性問題（新聞、天氣、股價、最新動態等）會自動上網查證後再回答
...
## 上網查資料（Google 搜尋 grounding）
呼叫 Gemini 時加了 `tools: [{ google_search: {} }]`，模型會自己判斷問題是否需要即時資訊，需要的話會自動查 Google 搜尋，再根據查到的結果回答，並在回覆最後附上參考來源連結（最多 3 個）。不需要另外申請搜尋 API 或金鑰，用的是 Gemini API 本身內建的功能，一般問答（不需要查資料的）不會觸發搜尋，速度跟原本一樣快。
